...
const formSchema = z
	.object({
		username: z.string({ invalid_type_error: "한글 혹은 영어만 입력 가능합니다.", required_error: INPUT_EMPTY_ERROR }).trim().toLowerCase().refine(checkUsername, "특수문자는 입력 불가능합니다."),
		email: z.string({ invalid_type_error: EMAIL_TYPE_ERROR, required_error: INPUT_EMPTY_ERROR }).toLowerCase().email({ message: EMAIL_TYPE_ERROR }),
